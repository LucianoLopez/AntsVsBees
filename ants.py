    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                elif insect.container == True and self.ant.container is False:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        self.other = other
        if self.container == True and self.ant is None and self.other.container == False:
            return True
        else:
            return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        "*** REPLACE THIS LINE ***"
        place = self.place
        while place.entrance is not hive and len(place.bees) == 0:
...
class LongThrower(ThrowerAnt):
    ...
    food_cost = 2
    def nearest_bee(self, hive):
        place, counter = self.place, 0
        while place.entrance is not hive:
            if counter >= self.min_range and len(place.bees) != 0:
                return random_or_none(place.bees)
            else:    
                place = place.entrance
                counter += 1
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        place, counter = self.place, 0
        while place.entrance is not hive:
            if counter <= self.max_range and len(place.bees) != 0:
                return random_or_none(place.bees)
            else:    
                place = place.entrance
                counter += 1
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        while place != None:
            if place.ant and place.ant.container == False:
                if place.ant and place.ant not in self.lst:
                    self.lst.append(place.ant)
                    place.ant.damage = place.ant.damage * 2
            elif place.ant and place.ant.ant != None:
                if place.ant not in self.lst:
                    self.lst.append(place.ant)
                    place.ant.damage = place.ant.damage * 2
                if place.ant.ant not in self.lst:
                    self.lst.append(place.ant.ant)
